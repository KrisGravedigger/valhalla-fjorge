...
def generate_nav_pnl_chart(snapshots_path: str | Path, output_dir: str | Path) -> None:
    """Generate combined portfolio NAV + cumulative investment chart."""
    ...
    # Read NAV snapshots (value_sol = total portfolio value)
    nav_points: list[tuple[datetime, Decimal]] = []
    with snapshot_file.open("r", newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            timestamp = row.get("timestamp", "").strip()
            value_sol = row.get("value_sol", "").strip()
            if not timestamp or not value_sol:
                continue
            try:
                nav_points.append((_parse_snapshot_timestamp(timestamp), Decimal(value_sol)))
    ...
    # Read capital flows for cumulative investment step function
    flows_path = Path(output_dir) / "capital_flows.csv"
    daily: dict[str, Decimal] = _defaultdict(lambda: Decimal("0"))
    if flows_path.exists():
        with flows_path.open("r", newline="", encoding="utf-8") as fh:
            for row in csv.DictReader(fh):
    ...
    ax.set_title(f"Portfolio NAV vs Investment — last {WINDOW_DAYS} days", fontsize=14, fontweight="bold")
    ax.set_ylabel("SOL", fontsize=11)
