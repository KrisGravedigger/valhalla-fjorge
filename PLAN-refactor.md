...
| **S4b-1..3** ⏳ | 2 | `cli.py`: monolit parse→report→backtest (linie ~219-961). **WYSOKIE ryzyko.** S4b-1 = Step 1 read/parse/dedup (pokryty przez --parse gate, `input()` w 293 verbatim). S4b-2 = Step 3 resolve. S4b-3 = Step 4 meteora + **duplikat w Step 8** (814-904 re-odpala match/merge/csv; unifikować w helper TYLKO jeśli byte-identyczny diff vs 468-513, inaczej = zmiana semantyki). Steps 3/4/8 częściowo/całkowicie verification-dark (sieć) → mogą wymagać mockowanych testów. `--merge`/`--recover-insuf` = churn, zostawić. `recalc` NIE istnieje w cli.py. wallet_trend (6.5c) wykluczony z baseline → dark bez własnego snapshotu. Thin wrappery (loss/charts/export/archive) zostawić. | **wysokie** | jw.; shim `valhalla_parser_v2.py` bez zmian |
