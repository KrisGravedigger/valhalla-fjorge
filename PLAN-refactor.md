...
| **S5-2** | 4 | `matcher.py`: ekstrakcja **czystych helperów** (low-risk): `_build_event_indexes()` (open_by_id/failsafe_ids/liquidity_by_id), `_sol_deployed(open, liquidity)`, `_meteora_pnl_pct(pnl, deposited)`. Verbatim, bez zmiany semantyki/importów. Chronione siatką S5-1. Delegacja do Codexa. | średnie | baseline + siatka zielone |
