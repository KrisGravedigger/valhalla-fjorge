...
| **S5** (rozbite na S5-1/2/3 — decyzja z advisorem) | 3+4 | `matcher.py`: in-file extraction `match_positions()` | **wysokie** | baseline + kontrakty zielone |
...
| **S5-3** | 4 | `matcher.py`: ekstrakcja **buildera `MatchedPosition`** (high-risk — 22 miejsca konstrukcji, subtelne różnice pól meteora/discord/pending oraz open/unknown). Chronione siatką S5-1 (pełnoobiektową). Delegacja do Codexa; rekomendowany `/codex:adversarial-review`. | **wysokie** | baseline + siatka zielone |
