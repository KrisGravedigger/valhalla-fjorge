...
| **S4b** ⏳ | 2 | `cli.py`: monolit parse→report→backtest (linie ~220-koniec). **WYSOKIE ryzyko** — parse oddaje duży worek lokalnego stanu (output_dir, event_parser, positions, args) do report; bundlowanie tego handoffu = miejsce, gdzie side-effect ordering (print/zapisy plików) cicho dryfuje. `--merge`/`--recover-insuf` (2-linijkowe delegacje) — opcjonalnie przy okazji, ale to churn nie redukcja. `recalc` NIE istnieje w cli.py (to `scripts/daily_recalc.py`). Rozbić na mikro-kroki jeśli za duże. | **wysokie** | jw.; shim `valhalla_parser_v2.py` bez zmian |
