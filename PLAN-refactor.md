...
| **S4b-3** ✅ *(2026-06-02, Step 4 only)* | 2 | `cli.py`: wydzielono Step 4 (Calculate Meteora PnL) z `main()` do helpera `_calculate_meteora_pnl(event_parser, args, resolved_addresses, already_complete_ids, already_meteora_ids)` → `(meteora_results, meteora_failed)`. Verbatim move (dedent only): fetch loop + closeable_ids + addresses_to_fetch + printy znak-w-znak (`end=''`/`flush=True`, `:.4f`, `${...:.2f}`). Gałąź fetch **verification-dark** (`--skip-meteora`) → diff review (w tym kolejność return — oba dicty puste w bramce, więc swap NIE byłby wykryty). Inicjalizatory pre-branch (no-input) zostają. **Step 8 NIE ruszony** (decyzja usera: osobny mikro-krok). Delegowane przez `codex exec`, check.ps1 zielony (272 passed, PnL 36.7321 SOL). Branch `claude/s4b1-parse-step1`. | **wysokie** | CLI działa identycznie; baseline zielony |
| **S4b-4** ✅ *(2026-06-02, rozstrzygnięte: NIE ruszać)* | 2 | `cli.py`: zbadano Step 8 (`if meteora_failed:` retry, ~835-924). **Diff Step 8 vs Step 4/5/6: NIE byte-identyczny** — to okrojona, pozbawiona printów, częściowa powtórka Stepów 5→6.5b (jedyny identyczny fragment: re-apply aliases vs Step 6.5a). Retry-fetch loop różni się od Step 4 (brak filtra closeable/addresses_to_fetch, inne printy/księgowanie). Ścieżka redo **pomija** Step 5.7 source-wallet, insuf/skip CSV, wykresy insuf+utilization, wallet_trend. **Decyzja użytkownika: zostawić Step 8 w main() (unifikacja = zmiana semantyki; verbatim-ekstrakcja = max ciężar review za min zysk, blok 100% dark).** Znaleziony utajony bug poprawności (retry → niepełny output) zapisany jako **TODO #148** (correctness, poza refaktorem). `matched_positions` itd. NIE czytane po Step 8 (potwierdzone). **S4b domknięty.** | — | bez zmian w kodzie; bramka zielona |
| **S5** ✅ *(domknięty 2026-06-03; S5-1+S5-2 done, S5-3 świadomie pominięty)* | 3+4 | `matcher.py`: in-file extraction `match_positions()` | **wysokie** | baseline + kontrakty zielone |
| **S5-1** ✅ *(2026-06-03)* | 3 | **Siatka pełnoobiektowa** (faza 3 "matcher — kontrakty", pogłębiona). `tests/test_matcher_characterization.py`: zamraża CAŁY `MatchedPosition` (każde pole przez `dataclasses.asdict`) dla **22 miejsc konstrukcji** + 5 scenariuszy cross-loop. Czysto addytywny, zero zmian produkcyjnych. **Domyka lukę pokrycia:** bramka `--parse` leci z `--skip-meteora` → KAŻDA gałąź `if meteora_result:` jest dark, a tylko 1 dotychczasowy test kontraktowy przekazywał `meteora_results`. Siatka ćwiczy każdy wariant meteora każdej pętli + 2 efekty uboczne nie-łapane przez close-only fixture: mutacja `resolved_addresses` (rug + already_closed) i akumulator `matched_ids`/kolejność pętli (w tym: **pętla rug NIE deduplikuje** → close+rug ten sam pid = 2 wiersze). EXPECTED wygenerowany przeciw kodowi pre-ekstrakcji (generator w `__main__`); regeneracja przy ekstrakcji = tautologia (zakazane). Napisane przez Claude Code (kontrakt weryfikacyjny — implementer ekstrakcji nie może autorować własnego grader-testu). Bramka zielona: 305 passed (+33), PnL 36.7321 SOL. Branch `claude/s5-matcher-extract`. | niskie | testy zielone przeciw obecnemu kodowi |
| **S5-2** ✅ *(2026-06-03)* | 4 | `matcher.py`: ekstrakcja **czystych helperów**: `_build_event_indexes()` (open_by_id/failsafe_ids/liquidity_by_id, 1 call), `_sol_deployed(open, liquidity)` (4 call-site), `_meteora_pnl_pct(pnl, deposited)` (8 call-site). Verbatim, Decimal byte-identyczny, bez zmiany importów/sygnatur. `.get(open_event.position_id)` == `.get(pid)` (open z open_by_id[pid]). Delegowane do Codexa przez `codex exec` fallback (plugin app-server „closed" — znany failure mode); diff przejrzany linia-po-linii przez Claude Code, bramka re-run poza sandboxem. 38/38 diff, czysta ekstrakcja. Bramka zielona: 305 passed, PnL 36.7321 SOL. Commit `8731508`. | średnie | baseline + siatka zielone |
| **S5-3** ⏭️ **ŚWIADOMIE POMINIĘTY** *(decyzja usera, 2026-06-03)* | 4 | `matcher.py`: builder `MatchedPosition` — **nie realizujemy.** **Powód:** to najryzykowniejszy krok o najmniej pewnym zysku. 22 miejsca konstrukcji różnią się na kilku osiach naraz (meteora/discord/pending × open-found/unknown-open × obecność pól meteora_* i target_wallet_address/tx). Unifikacja w jeden builder = funkcja z ~25 parametrami, która realnie **pogarsza** czytelność względem obecnych płaskich, czytelnych wywołań `MatchedPosition(...)`. S5-1 (siatka) + S5-2 (czyste helpery) zebrały już cały bezpieczny zysk czytelności matchera; sama repetycja konstrukcji nie jest defektem wartym ryzyka przepisania logiki finansowej. Nic od buildera nie zależy (S6+ niezależne), więc pominięcie nie blokuje dalszych etapów. Siatka S5-1 zostaje jako trwała wartość (regression-net). | — | n/d (pominięte) |
| **S6** ✅ *(domknięty 2026-06-03; S6-1+S6-2 done, dekompozycja pętli reguł świadomie pominięta)* | 5+6 | `merge.py`: in-file extraction (usunięcie duplikacji cross-function) | **wysokie (dane)** | baseline + kontrakty zielone |
| **S6-1** ✅ *(2026-06-03)* | 5 | **Celowana siatka** `tests/test_merge_characterization.py` (faza 5 „merge — kontrakty", uzupełnienie S2b). Zakres wąski wg advisora: baseline już diffuje positions.csv (S1.6) → pokrywa 28 pól rekonstrukcji w `merge_with_existing_csv` na ~13k wierszy; pełna siatka S5-style byłaby duplikacją. Dwie realne luki: (1) **round-trip 28 pól niepustych** (Rule-1 keep) — łapie pola zawsze puste w danych baseline (`source_wallet_*`, `original_wallet`), gdzie drop nie ruszyłby CSV; (2) **`merge_positions_csvs` jest w 100% dark** (tylko `--merge`, zero gate/testów) ORAZ to tam rekonstrukcja `position_id` różni się (stripped vs unstripped) — charakteryzacja dedup + reconstruction-fed Total PnL, napisana PRZED współdzieleniem helpera. Czysto addytywne. Bramka: 308 passed (+3). Commit `9c253db`. | niskie | testy zielone przeciw obecnemu kodowi |
| **S6-2** ✅ *(2026-06-03)* | 6 | `merge.py`: usunięcie duplikacji cross-function (177 del/94 ins). Hoist 5 parse-helperów do modułu; `_row_to_matched_position(row, position_id)` — **`position_id` jako jawny arg** (caller `merge_with_existing_csv` podaje stripped local, `merge_positions_csvs` podaje unstripped `row.get(...)` — asymetria zachowana byte-for-byte, krytyczne odkrycie advisora); `_open_event_to_matched_stub(event, position_id)` (3× powtórzenie + token_age norm). **Pominięte:** martwy `parse_decimal` w `merge_positions_csvs` (osobny cleanup), pętla reguł 1-4 + zagnieżdżone helpery (`is_fully_complete`/`normalize_specific_close_reason`/`enrich_existing_with_open` — już nazwane), wszystkie `print()`. Delegowane przez `codex exec` (fallback), diff przejrzany linia-po-linii, bramka poza sandboxem: 308 passed, PnL 36.7321 SOL. Commit `f9c8e0c`. | **wysokie (dane)** | baseline + siatka zielone |
| **S6-3 dekompozycja pętli reguł** ⏭️ **ŚWIADOMIE POMINIĘTA** *(2026-06-03)* | — | Dekompozycja pętli Rules 1-4 w `merge_with_existing_csv` — **nie realizujemy.** Powód (jak S5-3): to high-risk rozgałęzienie finansowe z **pinowanymi quirkami** (np. lpagent old-id double-count z S2b), a `is_fully_complete`/`normalize_specific_close_reason`/`enrich_existing_with_open` są już nazwanymi zagnieżdżonymi helperami. Rozbijanie tej logiki = duże ryzyko zmiany semantyki za niski zysk czytelności. S6 zebrał bezpieczny zysk (usunięcie duplikacji cross-function). | — | n/d (pominięte) |
| **S7** ✅ *(2026-06-04)* | 7 | `report_builder.py`: in-file extraction `generate_loss_report()` (~597 linii) → 9 modułowych rendererów `_render_*(...) -> List[str]` (header, exec summary, action items, recent losses, scorecard, filter recs, loss analysis 5a/5b/5c, global backtest, per-wallet) + cienki orkiestrator; hoisting `_fmt_age_hours`/`_fmt_age_threshold`/`PARAM_LABELS` do modułu. Jedyna cross-section zależność `active_action_items` (sec2→sec4) okablowana przez tuple-return. **Siatka świadomie pominięta** (jak S5-3/S6-3, inne uzasadnienie): cały output = `loss_analysis.md` diffowany **bajt-w-bajt** przez bramkę `--report` (brak „ciemnej siostrzanej funkcji" jak w S6, brak rekompilacji liczb — funkcja tylko renderuje gotowy `result`). Skan advisora: żadna zmienna nie jest czytana wyłącznie w dark-gałęzi. **Weryfikacja (4 niezależne):** (1) AST — każda nazwa Load w każdej funkcji/gałęzi jest param/lokalna/modułowa (wyłącza NameError w dark-gałęzi, mocniejsze niż celowany test); (2) `check.ps1` PASS niezależnie (py_compile + 308 pytest + `loss_analysis.md` bajt-w-bajt + parse, PnL 36.7321 SOL); (3) 111=111 `lines.append`, 1 plik, +108/−45; (4) differential `generate_loss_report([], …)` stary↔nowy = bajt-identyczny (pokrywa formatowanie fallbacków „brak danych" nieodpalanych przez baseline). Delegowane do Codexa (`codex exec` fallback, app-server pluginu „closed"), zweryfikowane przez Claude Code. Branch `claude/s7-report-builder`. | średni | baseline + AST + differential zielone |
| **S8** ✅ *(2026-06-04; S8-1 siatka + S8-2 ekstrakcja)* | 8 | `loss_analyzer.py`: in-file extraction `WalletScorecardAnalyzer.analyze()` — ostatni etap sekwencji. | średni | testy + AST + differential zielone |
| **S8-1** ✅ *(2026-06-04)* | 8 | **Siatka pełnoobiektowa** `tests/test_scorecard_characterization.py`: zamraża CAŁY `WalletScorecard` (29 pól przez `asdict`) dla 11 scenariuszy + guard. **Dlaczego siatka konieczna (S8≠S7, advisor):** `loss_analysis.md` renderuje tylko ~10 z 29 pól; część (`win_rate_24h/72h`, `capital_efficiency`, `consistency_score`, `median_pnl_sol`, okna `pnl_*d`/`rug_rate_*d`, `current_exposure_sol`) karmi **wyłącznie `wallet_trend.md`, wykluczony z baseline** → podwójnie dark, mis-wiring przeszedłby check.ps1. Fixture 100% syntetyczne (zero zależności od gitignorowanego `_baseline_pre_refactor/` — commitowany test self-contained). Wymiarowane empirycznie: realny baseline trafia wszystkie 5 gałęzi status, więc scenariusze trafiają każdą (`inactive`/`insufficient_data`/`increase_capital`/`consider_replacing` przez OBA pod-warunki pnl<0 i wr<próg/`monitor`) + brzegi (None-okna, capeff/avg_hold None, median parzysty/nieparzysty, exposure≠0, closed=0, rugi, sort cross-wallet). EXPECTED generowany przeciw kodowi pre-ekstrakcji (`__main__`). Napisane przez Claude Code (kontrakt weryfikacyjny). 12 testów, suite 320 passed. Commit `07ce17b`. | niskie | testy zielone przeciw obecnemu kodowi |
| **S8-2** ✅ *(2026-06-04)* | 8 | `loss_analyzer.py`: ekstrakcja ~230-liniowego ciała pętli per-wallet `analyze()` → metoda `_compute_wallet_scorecard(self, wallet, wallet_all, wallet_closed, reference_date, cutoff_7d/72h/24h/3d)`. 4 zagnieżdżone helpery (`is_win`/`_win_rate_recent`/`_window_closed`/`_rug_rate`) zostają zagnieżdżone (closure nad `wallet_closed`+cutoffs — instrukcja advisora, hoisting zerwałby closure); `RUG_REASONS` przeniesiony do helpera (używany tylko tam). `analyze()` = cienki orkiestrator. Verbatim, diff w całości wewnątrz `WalletScorecardAnalyzer`. Delegowane przez `codex exec` (fallback), zweryfikowane przez Claude Code. **Weryfikacja (5):** (1) tylko `loss_analyzer.py`, diff confined do jednej klasy; (2) scope-aware AST — zero nieokablowanych nazw w nowej metodzie/zagnieżdżonych helperach (5 flag = pre-istniejące lokalne importy `math`/`defaultdict`/`_date` w nietkniętych metodach); (3) siatka S8-1 zielona (12 passed); (4) `check.ps1` PASS (320 passed + `loss_analysis.md` bajt-w-bajt + parse); (5) **differential full-asdict stary↔nowy na realnym baseline CSV — 47 scorecardów × 29 pól, ZERO różnic w polach.** Wykryto pre-istniejącą niedeterministyczność kolejności listy (38 nieaktywnych walletów remisuje na `pnl_per_day_sol=0`, tie-break zależny od iteracji `set()` / PYTHONHASHSEED — identyczne w starym kodzie, filtrowane z raportu, NIE regresja S8; kandydat na osobny cleanup). Commit `46f37cc`. | **wysokie (finansowe)** | siatka + check + differential zielone |
