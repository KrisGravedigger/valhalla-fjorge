...
| **S4b-4** ⏳ *(Step 8 retry)* | 2 | `cli.py`: blok `if meteora_failed:` (retry failed Meteora, ~830-920). **Duplikuje** match→merge→csv ze Stepów 5-6. Przed jakąkolwiek unifikacją: **diff Step 8 vs Step 4/5/6 — unifikować w wspólny helper TYLKO jeśli byte-identyczny**, inaczej zostawić obie kopie (różnica = zmiana semantyki w przebraniu). Verification-dark (sieć, retry odpala się tylko gdy meteora_failed niepuste — nigdy w bramce). Najtrudniejszy kawałek S4b. `recalc` NIE istnieje w cli.py. wallet_trend (6.5c) wykluczony z baseline → dark. Thin wrappery (loss/charts/export/archive) zostawić. | **wysokie** | jw.; shim `valhalla_parser_v2.py` bez zmian |
