...
| **S1.6** ⟵ *następne (harden CSV gate — PRZED S5)* | — | domknąć lukę bramki CSV (patrz §3.1-bis): re-capture deterministycznego baseline `positions.csv`/`summary.csv` z czystego `--report`; decyzja o oknach `summary.csv` (kotwica do danych vs akceptacja artefaktu trybu `skips=0`); dodać `positions.csv` (min.) do `REPORT_FILES` | średnie | bramka chroni CSV; S5/S6 mają realną siatkę |
